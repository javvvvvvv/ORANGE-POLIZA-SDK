...
# ============================================================================
"""
Motor de fÃ³rmulas para Orange Poliza Engine.

Permite que cada empresa defina, por regla, cÃ³mo se calcula el importe
de cada movimiento de la pÃ³liza a partir de variables conocidas del
movimiento original (TOTAL, TASA_IVA, RET_IVA, etc.), usando una
expresiÃ³n de texto tipo:
...
No usamos eval() directo sobre el texto del usuario porque eso permitirÃ­a
ejecutar cualquier cÃ³digo Python. En vez de eso, parseamos la expresiÃ³n
con el mÃ³dulo `ast` y solo permitimos un subconjunto muy limitado de
nodos (nÃºmeros, nombres de variables conocidas, +, -, *, /, parÃ©ntesis,
signo negativo y la funciÃ³n round()). Cualquier otra cosa (imports,
...
class FormulaError(Exception):
    """Se lanza cuando una fÃ³rmula es invÃ¡lida o usa algo no permitido."""
    pass


# Operadores aritmÃ©ticos permitidos
...
# Funciones permitidas dentro de una fÃ³rmula
...
# Variables que el motor de reglas expone a cada fÃ³rmula.
...
VARIABLES_DISPONIBLES = [
    ...
    "TASA_IVA",    # ej. 0.16
    "BASE",        # TOTAL / (1 + TASA_IVA), calculado automÃ¡ticamente si hay IVA
    "IVA",         # TOTAL - BASE, calculado automÃ¡ticamente si hay IVA
    "RET_IVA",     # retenciÃ³n de IVA capturada manualmente (0 si no aplica)
    "RET_ISR",     # retenciÃ³n de ISR capturada manualmente (0 si no aplica)
    "TIPO_CAMBIO", # tipo de cambio si el movimiento es en dÃ³lares (1.0 si es en pesos)
]


def evaluar_formula(expresion: str, variables: dict) -> float:
    """
    EvalÃºa una fÃ³rmula de texto de forma segura y regresa el resultado
    redondeado a 2 decimales.

    :param expresion: texto de la fÃ³rmula, ej. "TOTAL / 1.16"
    :param variables: diccionario con los valores actuales del movimiento,
                       ej. {"TOTAL": 11600, "TASA_IVA": 0.16, ...}
    :raises FormulaError: si la fÃ³rmula no es vÃ¡lida o usa algo no permitido
    """
    if not expresion or not expresion.strip():
        raise FormulaError("La fÃ³rmula estÃ¡ vacÃ­a.")

    try:
        arbol = ast.parse(expresion, mode="eval")
    except SyntaxError as e:
        raise FormulaError(f"FÃ³rmula con error de sintaxis: {expresion!r} ({e})")

    resultado = _evaluar_nodo(arbol.body, variables)

    if not isinstance(resultado, (int, float)):
        raise FormulaError(f"La fÃ³rmula '{expresion}' no regresÃ³ un nÃºmero.")

    return round(float(resultado), 2)


def _evaluar_nodo(nodo, variables: dict):
    # NÃºmero literal: 1.16, 100, etc.
    if isinstance(nodo, ast.Constant):
        if isinstance(nodo.value, (int, float)):
            return nodo.value
        raise FormulaError(f"Valor no permitido en la fÃ³rmula: {nodo.value!r}")
    ...
    if isinstance(nodo, ast.Name):
        ...
        if nombre not in variables:
            ...
            raise FormulaError(
                f"La variable '{nombre}' no estÃ¡ disponible. "
                f"Variables disponibles: {disponibles}"
            )
        return variables[nombre]

    # OperaciÃ³n binaria: a + b, a - b, a * b, a / b
    if isinstance(nodo, ast.BinOp):
        ...
        if tipo_op is ast.Div and derecha == 0:
            raise FormulaError("DivisiÃ³n entre cero en la fÃ³rmula.")
        return _OPERADORES_BINARIOS[tipo_op](izquierda, derecha)

    # OperaciÃ³n unaria: -TOTAL, +TOTAL
    ...
    # ParÃ©ntesis ya vienen resueltos por el parser como parte del Ã¡rbol

    # Llamada a funciÃ³n: round(TOTAL / 1.16, 2)
    if isinstance(nodo, ast.Call):
        ...
        if nombre_funcion not in _FUNCIONES_PERMITIDAS:
            raise FormulaError(f"FunciÃ³n no permitida: {nombre_funcion}()")
        argumentos = [_evaluar_nodo(a, variables) for a in nodo.args]
        return _FUNCIONES_PERMITIDAS[nombre_funcion](*argumentos)

    raise FormulaError(f"Elemento no permitido en la fÃ³rmula: {type(nodo).__name__}")
...
def validar_formula(expresion: str) -> bool:
    """
    Valida que una fÃ³rmula sea sintÃ¡cticamente correcta y solo use
    variables conocidas, sin necesidad de tener valores reales todavÃ­a.
    Ãštil para validar en la interfaz mientras el usuario construye una regla.
    """
